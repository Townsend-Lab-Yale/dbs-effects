...
# Run from dbs-effects project directory (or change paths below) to use pre-generated MAF file.

# At time of writing, this version of cancereffectsizeR is a development version that includes
# features for working with DBS. If a newer version has been released, use it. Otherwise,
# you can reproduce the analysis by installing this version:
# remotes::install_github("Townsend-Lab-Yale/cancereffectsizeR@dbs-effects", dependencies = TRUE,
#     repos = BiocManager::repositories())

# Likewise:
# remotes::install_github("Townsend-Lab-Yale/ces.refset.hg38@dbs-effects")


stopifnot(packageVersion('cancereffectsizeR') >= as.package_version('3.0.0.9002'))
stopifnot(packageVersion('ces.refset.hg38') >= as.package_version('1.4.0.9000'))
...
skcm_maf = 'TCGA-SKCM_essential_columns.txt.gz' # Change path if necessary
...
mela = preload_maf(maf = skcm_maf, refset = 'ces.refset.hg38')
...
cosmic_class_count = table(factor(present_dbs$cosmic_dbs_class, levels = cancereffectsizeR:::cosmic_dbs_classes))
...
dbs_res = rbindlist(mapply(
  function(curr_dbs, num_with, only_rate) {
    ...
    ci = cancereffectsizeR:::univariate_si_conf_ints(fit, fn, min_si = 1e-3, max_si = 1e9, conf = .95)
...
# Verify claims cited in supplement
...
# Optionally, save analysis for future reference.
save_cesa(cesa, 'cesa_for_dbs.rds')

# Additional info for claims
